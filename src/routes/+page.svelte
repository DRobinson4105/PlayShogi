<script>
</script>

<div class="card rounded-none w-full h-full variant-filled-surface overflow-hidden">
	<div class="flex flex-row h-full">
		<div class="hero-text text-center w-1/2">
			<h1 class="text-5xl mt-32 mb-10">Play Shogi Online!</h1>
			<p class="text-base text-left inline-block ml-2">
				The customer is very important, the customer will be <br /> followed by the customer. Until
				then, he was a farmer.<br /> Children's biggest fear and concern is pregnancy.
			</p>
			<div class="signup/loginButton flex flex-row justify-center space-x-4 mt-12">
				<a href="/signup" class="btn variant-filled-primary heroButton w-3/12 border border-black"
					>Signup</a
				>
				<a href="/login" class="btn variant-filled-primary heroButton w-3/12 border border-black"
					>Login</a
				>
			</div>
			<div class="flex justify-center">
				<a href="/" class="btn variant-filled-tertiary heroButton w-7/12 border border-black mt-4"
					>Play as a Guest</a
				>
			</div>
		</div>
		<div class="hero-image w-1/2 flex justify-center items-center">
			<img src="/src/assets/shogi-board.png" alt="Shogi Board" class="w-3/4 h-3/4" />
		</div>
	</div>
</div>
